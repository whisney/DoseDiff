...
class UNetModel_MS_Former(nn.Module):
    """
    The full UNet model with attention and timestep embedding.
    ...
    def __init__(
        ...
        self.fusion = ViT_fusion(image_size=(image_size[0]//(2**(len(channel_mult)-1)), image_size[1]//(2**(len(channel_mult)-1))),
                                 patch_size=(image_size[0]//(2**(len(channel_mult))), image_size[1]//(2**(len(channel_mult)))),
                                 dim=1024, heads=1, mlp_dim=2048, channels=model_channels * channel_mult[-1], dim_head=64)
